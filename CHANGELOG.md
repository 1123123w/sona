...
- OP.GG 配装推荐接入 `/builds` 数据，装备页支持展示并写入第 4/5/6 件推荐装备。
- 智能配装新增默认推荐分路设置，用于客户端无法提供分路的模式。

### 改进

- 自动装备页改为按英雄唯一覆盖写入，避免同英雄在不同模式/分路下累积多个 Sona-E 装备页。
- 游戏内装备页导出改为仅关联当前地图，避免客户端按多地图导出多个 `RIOT_ItemSet` 副本。

### 修复

- 修复游戏内商店下拉出现历史 `[Sona]` / `[Sona-E]` 装备页残留的问题。
- 修复游戏内商店同英雄同模式同分路出现重复 Sona-E 装备页的问题。
- 修复客户端装备页与游戏内 Recommended 装备页双来源导致的重复显示问题。
...
- **????????** ? ???????????????? DOM/CSS ??? DataStore ?????? `sonaenhance`?
- **???????** ? ????? `sonaenhance:`??????????? `sona:` ????????????
- **???????** ? Release ????????????????? `1123123w/sona_enhance`?

## [1.3.2-rc.1] - 2026-05-19

### 🐛 Bug 修复

- **Counter 推荐窗口稳定性** — 修复选人阶段敌方头像绑定在客户端重绘、重启后可能失效的问题。
- **OP.GG 配装推荐入口稳定性** — 修复选人阶段短暂读取到空英雄上下文后，配装推荐入口可能被解绑的问题。
- **自动接受对局延迟取消** — 修复设置延迟后，玩家手动拒绝 ReadyCheck 仍会被定时器继续自动接受的问题。
